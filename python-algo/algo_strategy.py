...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def most_populated_quad(self, game_state):
        ...
        """ 
        #quad_counts [quad10_count, quad9_count, etc..]
        quad_counts = [0] * 10
        highest_den_quad = 0
    ...
    def most_populated_quad_col(self, game_state):
        """
        Determines which quadrant's column the enemy has most of their units in. The quadrants are divided into
        4 sections: divided between columns 7 and 8, 13 and 14, 19 and 20

        @param self
        @param game_state: obj repping the game's current state
        @return highest_quad_col: int repping most populated quadrant's column. Quad 1: 7, quad 2: 13,
                                  quad 3: 19, quad 4: 20
        """ 
        #quad_counts [quad1_count, quad2_count, quad3_count, quad4_count]
        quad_counts = [0] * 4
        ...
        for location in game_state.game_map:
            if game_state.contains_stationary_unit(location) and location[1] >= 14:
                ...
                if location[0] <= 7:                # Q1
                    quad_counts[0] += len(units)
                elif location[0] <= 13:             # Q2
                    quad_counts[1] += len(units)
                elif location[0] <= 19:             # Q3
                    quad_counts[2] += len(units)
                else:                               # Q4
                    quad_counts[3] += len(units)

        # Determining which quad has the highest amount of units
        max_quad_ind = quad_counts.index(max(quad_counts))

        # Returning the appropriate unit
        if max_quad_ind == 0:
            return 7
        elif max_quad_ind == 1:
            return 13
